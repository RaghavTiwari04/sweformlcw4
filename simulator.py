...
def run_mllp_server(host, port, hl7_messages, shutdown_mllp, short_messages):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while not shutdown_mllp.is_set():
            ...
            t = threading.Thread(target=serve_mllp_client, args=(
                client, source, hl7_messages, shutdown_mllp, short_messages), daemon=True)
            t.start()
...
def main():
    ...
    parser.add_argument("--short_messages", default=False, action="store_true",
                        help="Encourage all outgoing messages to be split in two")
    flags = parser.parse_args()
    hl7_messages = read_hl7_messages(flags.messages)
    shutdown_event = threading.Event()
    mllp_thread = threading.Thread(target=run_mllp_server, args=(
        "0.0.0.0", flags.mllp, hl7_messages, shutdown_event, flags.short_messages), daemon=True)
    mllp_thread.start()
    pager = None
    ...
    pager_thread = threading.Thread(target=pager.serve_forever, args=(), kwargs={
                                    "poll_interval": SHUTDOWN_POLL_INTERVAL_SECONDS}, daemon=True)
    pager_thread.start()
    mllp_thread.join()
    pager_thread.join()
